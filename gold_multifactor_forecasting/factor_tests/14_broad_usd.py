...
def build_features(df, market):
    if "broad_usd" not in df.columns:
        return {}
    x = pd.to_numeric(df["broad_usd"], errors="coerce")
    return {
        "usd_ret5": log_return(x, 5),
        "usd_ret20": log_return(x, 20),
        "usd_z252": rolling_zscore(x, 252, 80),
    }
