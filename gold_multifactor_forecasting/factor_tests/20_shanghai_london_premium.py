...
FAMILY = "shanghai_london_premium"


def build_features(df, market):
    if market != "shanghai" or "shanghai_london_premium" not in df.columns:
        return {}
    x = pd.to_numeric(df["shanghai_london_premium"], errors="coerce")
    return {
        "premium_level": x,
        "premium_change5": x - x.shift(5),
        "premium_change20": x - x.shift(20),
        "premium_z252": rolling_zscore(x, 252, 80),
    }
